...
class BasicTrainer(Trainer):
    ...
    def init_models_and_more(self, **kwargs):
        ...
        if self.world_size > 1:
            # Prepare distributed data parallel. find_unused_parameters defaults
            # to False; subclasses whose forward graph varies per iteration
            # (e.g. temporal trainers with optional prev context) set the
            # class attribute DDP_FIND_UNUSED_PARAMETERS = True.
            find_unused = getattr(self, 'DDP_FIND_UNUSED_PARAMETERS', False)
            static_graph = getattr(self, 'DDP_STATIC_GRAPH', False)
    ...
    def load(self, load_dir, step=0, reset_optimizer=False):
        """
        Load a checkpoint.
        Should be called by all processes.

        Args:
            reset_optimizer: if True, skip loading the optimizer state. Use
                when the trainable scope has changed (e.g. self_attn ->
                transformer) since Adam moments are shape-keyed to the old
                param set and aren't valid for the new one.
        """
        if self.is_master:
        ...
        if reset_optimizer:
            if self.is_master:
                print(f' [load] reset_optimizer=True — skipping optimizer state.', end='')
        else:
            try:
                self.optimizer.load_state_dict(misc_ckpt['optimizer'])
                # Sanity-check shapes: PyTorch's load_state_dict only checks
                # param-group counts, not per-param tensor shapes. If the
                # trainable scope changed, the load can silently succeed and
                # then blow up at the first step() with a foreach_lerp_ size
                # mismatch. Catch that here.
                for group in self.optimizer.param_groups:
                    for p in group['params']:
                        st = self.optimizer.state.get(p, {})
                        if 'exp_avg' in st and st['exp_avg'].shape != p.shape:
                            raise RuntimeError(
                                f'optimizer exp_avg shape {tuple(st["exp_avg"].shape)} != param shape {tuple(p.shape)}'
                            )
            except (ValueError, RuntimeError) as e:
                if self.is_master:
                    print(f'\n[load] optimizer state incompatible — initializing fresh: {e}', end='')
